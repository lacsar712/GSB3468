<template>
  <div class="manage-page">
    ...
    <el-table :data="items" stripe style="width: 100%" v-loading="loading">
      ...
      <el-table-column prop="speciality" label="擅长" width="150" />
...
const fetchItems = async () => {
  loading.value = true
  try {
    const res = await axios.get('/api/friends', { params: { page: page.value - 1, size: pageSize } })
...
const deleteItem = async (row) => {
  try {
    await ElMessageBox.confirm(`确定删除钓友「${row.name}」吗？`, '删除确认', { type: 'error' })
    await axios.delete(`/api/friends/${row.id}`)
