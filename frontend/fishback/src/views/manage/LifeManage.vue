...
const fetchItems = async () => {
  loading.value = true
  try {
    const res = await axios.get('/api/life', { params: { page: page.value - 1, size: pageSize } })
...
const deleteItem = async (row) => {
  try {
    await ElMessageBox.confirm(`确定删除「${row.title}」吗？`, '删除确认', { type: 'error' })
    await axios.delete(`/api/life/${row.id}`)
